...
### 云端与本地的一处行为差异：token 统计不持久

控制台的「统计」页在云端**每次部署/重启都会清零**，本地则不会。

原因是云端没有可写磁盘：统计需要有人在进程外存一份（CF 的 KV/D1、Vercel 的外部存储），
而本项目刻意保持"单文件复制粘贴即可部署"，不引入这些依赖。云端下统计只活在当前实例内存里，
且多个 isolate / 多区域时各记各的，数字会偏低。

- 想要跨重启的用量账本 → 在**本地**跑（统计自动写到 `~/.cline-free-usage.local.json`）。
- 只想知道"今天用了多少" → 云端够用，额度本身就是按自然日重置的。

> 🔓 `GET /v1/health` **不鉴权**，其中的 `usage` 字段会公开你的 token 用量规模。
> 部署到公网且在意这一点时，请在 `worker.js` 里把 usage 从 health 响应中摘掉，
> 或改成只对带 `API_KEY` 的请求返回。
